...
const File = ({ appState }: { appState: string }) => {
  ...
  const getFile = useCallback(async () => {
    if (!id) return;
    try {
      const fileMet      export type FileType = {
        id: string;
        fileName: string;
        checksum: string;
        creationDate: string;
  ...
  if (!file) return null;
  return (
    <Descriptions.Item label="File Data" column={1} bordered={true}>
      ...
      <Descriptions.Item label="Creation Date">
        {file.creationDate ? new Date(file.creationDate).toLocaleString() : 'Invalid Date'}
      </Descriptions.Item>
